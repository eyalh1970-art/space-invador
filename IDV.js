// ============================================================
//  IDV – Space Invaders: All 3 Levels United
// ============================================================
...
function drawEnemies() {
  for (const e of enemies) {
    ...
    if (currentLevel === 2) {
      ...
      ctx.fillText(label, e.x+e.w/2, e.y-11);
    } else {
      ...
      ctx.fillStyle = color;
      if (img.loaded) ctx.drawImage(img, e.x-10, e.y-8, e.w+20, e.h+16);
...
function drawUFO() {
  ...
  if (img.loaded) ctx.drawImage(img, ufo.x, ufo.y-6, ufo.w, ufo.h+12);
  else {
  ...
  ctx.fillText(label, ufo.x+ufo.w/2, ufo.y-8);
}
